...
export class Statements<T> {
     ...
     * @returns {Promise<D1Result<T> | Response>} - The response from the database.
     */
    async get(): Promise<D1Result<T> | Response> {
        await this.checkTemplate();
        const statement = `SELECT * FROM ${this.table}`;
        if (!this.body || this.isEmpty()) { return await this.DB.prepare(statement).all() as D1Result<T>; }
        if (!this.onlyOneProvided()) { return this.error("Only one parameter allowed"); }
        const allProps = (this.body as any);
        const entry = Object.entries(allProps)[0];
        if (!entry) { return this.error("No properties found"); }
        const [key, value] = entry;
        const res: D1Result<T> = await this.DB.prepare(`${statement} WHERE ${key} = ?`).bind(value).all();
     ...
     * @returns {Promise<D1Result<T> | Response>} - The response from the database.
     */
    async add(): Promise<D1Result<T> | Response> {
        await this.checkTemplate();
     ...
     * @returns {Promise<D1Result<T> | Response>} - The response from the database.
     */
    async update(): Promise<D1Result<T> | Response> {
        await this.checkTemplate();
     ...
     * @returns {Promise<D1Result<T> | Response>} - The response from the database.
     */
    async delete(): Promise<D1Result<T> | Response> {
        await this.checkTemplate();
        const id = this.getBodyId();
        if (typeof id === "string") { return this.error(id); }
     ...
     * @returns {Promise<D1Result<T>>} - The response from the database.
     * @private
     */
    private async prepare(statement: string, bindObject: any = null): Promise<D1Result<T> | Response> {
        try {
            if (statement.endsWith(", )")) { statement = statement.replace(", )", ", NULL)"); }
            if (!bindObject) { return await this.DB.prepare(statement).run(); }
            return await this.DB.prepare(statement).bind(bindObject.join(", ")).run();
        }
        catch (error: any) {
